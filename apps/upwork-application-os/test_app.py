...
class UpworkOSTest(unittest.TestCase):
    def setUp(self):
        ...
        self.job = {"source_url":"https://www.upwork.com/jobs/~test123","external_id":"test123","title":"Klaviyo lifecycle automation","description":"Improve retention and win-back flows","requirements":"","is_open":True,"missing_required_facts":[],"unsupported_proof_required":False}
    ...
    def test_score_is_apex(self):
        score, tier = app.score_job(self.parts, self.config); self.assertEqual(score, 89); self.assertEqual(tier, "APEX")

    def test_canonical_url_identity_ignores_query_and_external_id_presence(self):
        a = app.canonical_job_key("https://www.upwork.com/jobs/~ABC123?source=rss", "different-id")
        b = app.canonical_job_key("https://www.upwork.com/jobs/~ABC123/", None)
        self.assertEqual(a, b)

    def test_closed_job_blocked(self):
        self.assertEqual(app.hard_gate(dict(self.job, is_open=False), self.config, []), "JOB_CLOSED")

    def test_duplicate_blocked(self):
        key = app.canonical_job_key(self.job["source_url"], self.job["external_id"])
        self.assertEqual(app.hard_gate(self.job, self.config, [{"job_key":key,"state":"SUBMITTED"}]), "DUPLICATE")

    def test_terminal_history_still_blocks_reapply(self):
        key = app.canonical_job_key(self.job["source_url"], self.job["external_id"])
        for state_name in ("WON","LOST","SUPPRESSED","REJECTED"):
            with self.subTest(state=state_name): self.assertEqual(app.hard_gate(self.job,self.config,[{"job_key":key,"state":state_name}]),"DUPLICATE")

    def test_missing_fact_stops_submission(self):
        r = app.prepare_record(dict(self.job, missing_required_facts=["requested case-study metric"]), self.parts, proposal="draft")
        self.assertEqual(r["state"], "NEEDS_HUMAN_FACT")

    def test_unsupported_proof_rejected(self):
        r = app.prepare_record(dict(self.job, unsupported_proof_required=True), self.parts, proposal="draft")
        self.assertEqual(r["state"], "REJECTED"); self.assertEqual(r["hard_gate_reason"], "UNSUPPORTED_PROOF_REQUIRED")
    ...
    def test_suppression_can_stop_ready_job(self):
        with tempfile.TemporaryDirectory() as d:
            p=Path(d)/"queue.json"; p.write_text(json.dumps([{"job_key":"x","state":"READY_TO_SUBMIT"}]))
            with patch.object(status,"QUEUE_PATH",p): row=status.transition("x","SUPPRESSED",suppression_reason="manual stop")
    ...
    def test_metrics(self):
        ...
        self.assertEqual(m["submitted"],2); self.assertEqual(m["wins"],1); self.assertEqual(m["revenue_per_connect"],50)
