...
function inColdWindow() { const m = londonMinutesNow(); return m >= 480 && m <= 570; }
...
function parseControl(text) {
  ...
  if (a >= 0 && b > a) { try { return JSON.parse(v.slice(a, b + 1)); } catch {} }
  return null;
}
function expandJobs(control) {
  ...
  if (Array.isArray(control.jobs)) {
    ...
    return control.jobs.map((job) => ({ ...job, dispatchAuth: job?.dispatchAuth || control.dispatchAuth }));
  }
  return [{ ...control }];
}
function loadLedger() {
  ...
  try {
    ...
    return { version: 1, updatedAt: parsed.updatedAt || null, idempotency: parsed.idempotency && typeof parsed.idempotency === 'object' ? parsed.idempotency : {}, daily: parsed.daily && typeof parsed.daily === 'object' ? parsed.daily : {} };
  } catch (error) { throw new Error(`Direct ledger is unreadable: ${error?.message || error}`); }
}
...
async function executeJob(job, location) {
  ...
  let info;
  try {
    info = await transporter.sendMail({ from: `Chelston Phillip <${USER}>`, to: [recipient], subject: job.subject, text: job.text, replyTo: USER, headers: { 'X-TTE-Lead-ID': String(job.leadId), 'X-TTE-Touch-No': String(job.touchNo), 'X-TTE-Idempotency-Key': String(job.idempotencyKey), 'X-TTE-Route': 'mailopoly-control-imap-github-privateemail' } });
  } catch (error) {
    ledger.idempotency[job.idempotencyKey] = { ...ledger.idempotency[job.idempotencyKey], state: 'DELIVERY_PENDING', failedAt: new Date().toISOString(), code: error?.code || null, responseCode: Number(error?.responseCode || 0) || null, message: error?.message || 'SMTP send failed' };
  ...
  if (!accepted.includes(recipient) || rejected.includes(recipient)) {
    ledger.idempotency[job.idempotencyKey] = { ...ledger.idempotency[job.idempotencyKey], state: 'DELIVERY_PENDING', messageId: info.messageId || null, accepted, rejected, failedAt: new Date().toISOString() };
    try { persistLedger(`hold ${job.idempotencyKey}`); } catch {}
    return { status: 'DELIVERY_PENDING' };
  }
  ...
  try { persistLedger(`confirm ${job.idempotencyKey}`); } catch (error) { console.error(`TTE queue ${location} SMTP accepted but final ledger push failed. Fail closed on next run. idempotency=${job.idempotencyKey}: ${error?.message || error}`); return { status: 'DELIVERY_PENDING', body: result }; }
...
try {
  ...
  for (const mailbox of selectable) {
    ...
    await client.mailboxOpen(mailbox.path, { readOnly: true }); scannedFolders += 1;
    const since = new Date(Date.now() - LOOKBACK_MS);
    const recent = await client.search({ since }, { uid: true }); const uids = recent.slice(-100); if (!uids.length) continue;
    const messages = await client.fetchAll(uids, { uid: true, envelope: true, source: true }, { uid: true });
    for (const message of messages) {
      const from = message.envelope?.from?.[0]?.address?.toLowerCase() || ''; const subject = message.envelope?.subject || '';
      if (!CONTROL_SENDERS.has(from) || !PREFIXES.some((prefix) => subject.startsWith(prefix))) continue;
      candidates += 1; const location = `folder=${mailbox.path} uid=${message.uid}`;
      const parsed = await simpleParser(message.source, { skipTextToHtml: true, maxHtmlLengthToParse: 150000 });
      const control = parseControl(parsed.text || ''); const jobs = expandJobs(control);
      if (!jobs.length) { console.error(`TTE queue: malformed or oversized control ${location}`); blocked += 1; continue; }
