...
function parseClockOnDate(dateIso, hhmm) {
...
export function freeWindows(state, config) {
  ...
  const dayStart = parseClockOnDate(now, config.defaultWorkday.start);
  const dayEnd = parseClockOnDate(now, config.defaultWorkday.end);
...
export function planCalendar(state, evaluated, config) {
  ...
  for (const task of evaluated.ranked) {
    ...
    while (remaining > 0 && wi < windows.length && used < usableBudget) {
      ...
      const aosId = crypto.createHash('sha256').update(`${task.id}|${start.toISOString()}|${end.toISOString()}`).digest('hex').slice(0, 24);
      blocks.push({
        aosId,
        taskId: task.id,
